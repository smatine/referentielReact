...
class VpcList extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`/accounts/${this.props.match.params.id}/vpcs`)
  ...
  render() {
    ...
    const vpcList = vpcs.map(vpc => {
      ...
      const link = "/product/" +  vpc.account.product.id + "/accounts/" + vpc.account.id; 
      ...
      return <tr key={vpc.id}>
        ...
        <td><a href={link}>{vpc.account.numAccount}</a></td>
		    <td>{vpc.text}</td>
		
        <td>
          <ButtonGroup>
            <Button size="sm" color="secondary" tag={Link} to={"/vpc/" + vpc.id + "/subnets"}>Subnets</Button>&nbsp;&nbsp;
          </ButtonGroup>
        </td>

        <td>
          <ButtonGroup>
            <Button size="sm" color="primary" tag={Link} to={"/account/" + vpc.account.id + "/vpcs/" + vpc.id}>Edit</Button>
   ...
   const add = `/account/${this.props.match.params.id}/vpcs/new`;
