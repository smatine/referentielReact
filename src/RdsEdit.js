...
class RdsEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.match.params.id !== 'new') {
      const rds = await (await fetch(`/rdss/${this.props.match.params.id}`)).json();
      rds.touched = {
        name: false,
        vpcId: false,
        subnetgroupId: false
      ...
      fetch('/vpcs/' + (rds.vpc.id) + '/type/RDS' + '/subnetGroups',)
        .then((result) => {
          return result.json();
        }).then((jsonResult) => {
  ...
  handleChange(event) {
    ...
    if(name === 'vpcId')
    {
      ...
      .then((result) => {
        ...
        fetch('/vpcs/' + (vp.id) + '/type/RDS' + '/subnetGroups',)
        .then((result) => {
          return result.json();
        }).then((jsonResult) => {
          item.subnetgroups = jsonResult;
  ...
  async handleSubmit(event) {
    ...
    await fetch((item.id) ? '/vpcs/' + (item.vpc.id) + '/rdss/'+(item.id) : '/vpcs/' + item.vpc.id + '/rdss', {
