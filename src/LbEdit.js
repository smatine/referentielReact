...
class LbEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.match.params.id !== 'new') {
      ...
      const lb = await (await fetch(`/lbs/${this.props.match.params.id}`)).json();
      lb.touched = {
        name: false,
        vpcId: false,
        type : false,
      ...
      let vp={};
      await (await fetch("/vpcs/" + lb.vpc.id,)
      .then((result) => {
        return result.json();
      }).then((jsonResult) => {
        vp = jsonResult;
  ...
  async handleSubmit(event) {
    ...
    await fetch((item.id) ? '/vpcs/' + (item.vpc.id) + '/lbs/'+(item.id) : '/vpcs/' + item.vpc.id + '/lbs', {
  ...
  validate(name, vpcId, type, scheme, ipType, subnetId, subnets, sgId) {
